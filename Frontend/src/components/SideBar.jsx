...
const SideBar = () => {
    ...
    const sidebarLinks = [
        { name: "Add Product", path: `/seller/${adminId}/dashboard`, icon: dashboardicon },
        { name: "Product List", path: `/seller/${adminId}/product-list`, icon: overviewicon },
        { name: "Orders", path: `/seller/${adminId}/orders`, icon: chaticon },
    ];

    const currentPath = location.pathname;
    const isDashboard = currentPath === `/seller/${adminId}/dashboard`;
    const isProductList = currentPath === `/seller/${adminId}/product-list`;
    ...
    return (
        ...
        <div className='flex'>
                ...
                {sidebarLinks.map((item, index) => {
                    ...
                    return (
                        <Link to={item.path} key={index}
                            className={`flex items-center py-3 px-4 gap-3 ${isActive ? "border-r-4 md:border-r-[6px] bg-primary/10 border-primary" : "hover:bg-gray-100/90 border-white text-gray-700"}`}
                        >
                            {item.icon}
            ...
            {isDashboard ? <AddProduct /> : isProductList ? <PList /> : <AllOrders />}
        </div>
    </>
  )
}
